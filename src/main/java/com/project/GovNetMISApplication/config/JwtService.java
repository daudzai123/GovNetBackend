...
import com.project.GovNetMISApplication.ExceptionHandlingFiles.InvalidTokenException;
...
public class JwtService {
        ...
        return userDetails.getAuthorities()
                ...
                .map(authority -> {
                    if (authority instanceof Roles) {
                        return "ROLE_"+((Roles) authority).getRoleName();
                    } else {
    ...
    claims.put("roles", "User Roles" ); // Add roles to claims getRolesFromUserDetails(userDetails) getRolesFromUserDetails(userDetails)
...
private Claims extractAllClaims(String token) {
    try {
        return Jwts
                ...
                .setSigningKey(getSignInKey())
                .build()
                .parseClaimsJws(token)  // Correct method for JWS
                .getBody();
